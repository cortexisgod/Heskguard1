...
client.on('guildUpdate', async (oldGuild, newGuild) => {
  let cortex = await newGuild.fetchAuditLogs({type: 'GUILD_UPDATE'}).then(audit => audit.entries.first());
  if (!cortex || !cortex.executor || güvenli(cortex.executor.id)) return ;
  punish(cortex.executor.id, "ban");
  ...
  client.channels.cache.get(config.Log.GuildUpdateLog).send(new MessageEmbed().setTitle('**Sunucu Güncellendi**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` sunucuyu güncelledi. \n Sunucuyu eski haline çevirdim ve güncelleyen kişiyi banladım.`))
})

client.on('guildMemberAdd', async member => {
  let cortex = await member.guild.fetchAuditLogs({type: 'BOT_ADD'}).then(audit => audit.entries.first());
  if (!member.user.bot || !cortex || !cortex.executor || güvenli(cortex.executor.id)) return;
  punish(cortex.executor.id, "ban");
  punish(member.id, "ban");
  client.channels.cache.get(config.Log.BotAddLog).send(new MessageEmbed().setTitle('**Bir Bot Eklendi**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` Tarafından bot eklendi. \n ${cortex.executor} ve ${member} banladım.`).addField('Eklenen Bot', member))
})

client.on("guildMemberRemove", async member => {
  let cortex = await member.guild.fetchAuditLogs({type: 'MEMBER_KICK'}).then(audit => audit.entries.first());
  if ( !cortex || !cortex.executor || güvenli(cortex.executor.id)) return;
  punish(cortex.executor.id, "ban")
  client.channels.cache.get(config.Log.KickLog).send(new MessageEmbed().setTitle('**Üye Tarafından Kick Atıldı**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` Tarafından kick atıldı.`).addField('Kicklenen Üye', member))
})

client.on('guildBanAdd', async (guild, user) => {
  let cortex = await guild.fetchAuditLogs({type: 'MEMBER_BAN_ADD'}).then(audit => audit.entries.first());
  if (!cortex || !cortex.executor || güvenli(cortex.executor.id)) return;
  punish(cortex.executor.id, "ban");
  ...
  client.channels.cache.get(config.Log.BanLog).send(new MessageEmbed().setTitle('**Üye Tarafından Ban Atıldı**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` Tarafından ban atıldı. \n Ban atan kişiyi banladım ve banlanan kişinin banını kaldırdım.`).addField('Banlanan Üye', user))

})

client.on('roleCreate', async role => {
  let cortex = await role.guild.fetchAuditLogs({type: 'ROLE_CREATE'}).then(audit => audit.entries.first());
  if (!cortex || !cortex.executor || güvenli(cortex.executor.id)) return;
  role.delete({reason: "Guard Bot Tarafından Silidnid"})
  punish(cortex.executor.id, "jail")
  client.channels.cache.get(config.Log.RoleLog).send(new MessageEmbed().setTitle('**Bir Rol Oluşturuldu**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` Tarafından rol oluşturuldu \n Rolü oluşturan kişiyi Jail Attım ve Oluşturulan rolü sildim.`))
})

client.on('channelCreate', async channel => {
  let cortex = await channel.guild.fetchAuditLogs({type: 'CHANNEL_CREATE'}).then(audit => audit.entries.first());
  if (!cortex || !cortex.executor || güvenli(cortex.executor.id)) return;
  channel.delete({reason: 'Guard Bot Tarafından Silindi'})
  punish(cortex.executor.id, "jail")
  client.channels.cache.get(config.Log.ChannelLog).send(new MessageEmbed().setTitle('**Bir Kanal Oluşturuldu**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` Tarafından Kanal oluşturuldu \n Kanalı oluşturan kişiyi Jail Attım ve Oluşturulan kanalı sildim.`))
})

client.on("channelDelete", async channel => {
  let cortex = await channel.guild.fetchAuditLogs({type: 'CHANNEL_DELETE'}).then(audit => audit.entries.first());
  if (!cortex || !cortex.executor || güvenli(cortex.executor.id)) return;
  punish(cortex.executor.id, "ban");
  ...
  client.channels.cache.get(config.Log.ChannelLog).send(new MessageEmbed().setTitle('**Bir Kanal Silindi**').setColor("RED").setTimestamp().setDescription(`${cortex.executor} \`${cortex.executor.id}\` Tarafından kanal silindi. \n Kanalı silen kişiyi banladım ve kanalı yeniden oluşturdum. (Deleted Yazıyorsa Denetim Kaydını İnceleyiniz) \n Geri oluşturulan kanalın ayarlarını düzenleyiniz.`).addField('Silinen Kanal', channel))
